...
- **Note on real fixtures**: The two committed fixtures (`alpha.yaml` and `beta.yaml`) do NOT have any proxy-group name collisions in the wild — alpha uses emoji-prefixed Chinese names like `🔰国外流量` while beta uses plain Chinese names like `synthetic-pool`. The integration test injects a synthetic same-named group across both stubs to exercise the merge logic, since real-world same-name collisions are infrequent but the behavior must still be correct.
